...
import { createSeries, deleteSeriesById, getSeriesByProducerId, getSeriesById, submitSeriesForReview, updateSeriesById } from '../../services/series.service';
...
export const createProducerSeries = async (req: Request, res: Response) => {
  ...
  if (!producerDoc.exists) return res.status(404).json({ message: 'Producer not found' });

  try {
    const series = await createSeries(req.body, producerId, producerDoc.data());
    return res.status(201).json({ success: true, data: series });
  } catch (err: any) {
    return res.status(422).json({ success: false, error: { code: err.code || 'unknown', message: err.message } });
  }
};
...
export const getProducerSeries = async (req: Request, res: Response) => {
  ...
  if (!series || series.producerId !== producerId) return res.status(403).json({ message: 'Forbidden' });
  return res.json({ success: true, data: series });
};
